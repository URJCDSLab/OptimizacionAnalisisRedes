...
def check_env():
    ...
    if not os.path.exists("apuntes"):
        print("[ERROR] No se encuentra la carpeta 'apuntes'.")
        return False
        
    # Verificar archivos necesarios
    archivos = [
        "apuntes/portada.qmd",
        "apuntes/portada.html",
        "_quarto.yml",
        "estilos.css"
    ]
...
def crear_libro():
    ...
    try:
        # Renderizar el libro completo en la carpeta apuntes/apuntes_pdf
        subprocess.run(["quarto", "render", "--to", "pdf", "--output-dir", "apuntes/apuntes_pdf"], env=get_conda_env(), check=True)
