...
\begin{frame}{Modelo de flujo: camino mínimo $\mu^*[s,t]$}
   
\begin{itemize}
    \item {\bf Datos:} $(G=(V,U), \{ d_u, u \in U \} )$
    \item {\bf Variables:}
	$$x_{u} = \begin{cases}1, & \textrm{si se usa el arco $u\in U$ en el camino mínimo,}
    \\
    0, & \textrm{si no se usa.}  
    \end{cases}$$
    \item {\bf Restricciones:}
     \begin{itemize}
      \item Nodos intermedios $i\in V\setminus\{s,t\}$: entra 1 arco y sale 1 arco:
         $$
     \sum_{u\in \Gamma^-(i)} x_u - \sum_{u\in \Gamma^+(i)} x_u=0
     $$
         \item De $s\in V$ sólo sale un arco:
         $$
     \sum_{u\in \Gamma^-(s)} x_u - \sum_{u\in \Gamma^+(s)} x_u=-1
     $$
         \item A $t\in V$ sólo llega un arco:
         $$
     \sum_{u\in \Gamma^-(t)} x_u - \sum_{u\in \Gamma^+(i)} x_u=1
     $$       
     \end{itemize}   
...
\begin{frame}{Modelo de flujo: camino mínimo $\mu^*[s,t]$}
	...
	\begin{align*}
		...
		\textrm{s.a.:\;\;} &  \sum_{u\in \Gamma^-(s)}x_{u} - \sum_{u\in \Gamma^+(s)} x_{u}  = -1 \\
		&  \sum_{u\in \Gamma^-(t)} x_{u} - \sum_{u\in \Gamma^+(t)}x_{u} = 1\\
		&   \sum_{u\in \Gamma^-(i)} x_{u} - \sum_{u\in \Gamma^+(i)}x_{u}=0\qquad \forall i\neq s,t \\
		&   x_{u} \in \{0,1\}, \; u\in U
	\end{align*}
...
{\tiny 
    \begin{center}
      ...
      %  \\
        \hline
        Arco & $(s,1)$ & $(s,2)$ & $(1,2)$ & $(1,3)$ & $(1,4)$ & $(2,1)$ & $(2,3)$ & $(2,5)$ &
        $(3,4)$ & $(3,5)$ & $(3,t)$ & $(4,t)$ & $(5,t)$ 
        \\
        Dist. & 5 &	4 &	4 &	2 &	7 &	3 &	4 &	5 &	7 &	7 &	13 & 5 & 2
        \\
        \hline
        \end{tabular}
    \end{center}
...
\text{Min} & \multicolumn{14}{l}{5x_{s1}+	4x_{s2}+	4x_{12}+	2x_{13}+	7x_{14}+	3x_{21}+	4x_{23}+	5x_{25}+	7x_{34}+	7x_{35}+	13x_{3t}+	5x_{4t}+	2x_{5t}}
\\
\text{s.a.} & -x_{s1} & - x_{s2} & & & & & & & & & & & & = -1
\\
& +x_{s1} & &-x_{12} & -x_{13} & -x_{14} & +x_{21} & & & & & & &  & =  0
\\
&  & +x_{s2} &+x_{12} &  &  & -x_{21} & -x_{23} & -x_{25} & & & & &  & =  0
\\
&  &  & & +x_{13} &  &  & +x_{23} &  & -x_{34} & -x_{35} & -x_{3t} & &  & =  0
\\
&  &  & &  & +x_{14}  &  &  &  & +x_{34} &  &  & -x_{4t} &  & =  0
...
&  &  & &  &  &  &  &  &  &   & +x_{3t}  & +x_{4t} & +x_{5t} & =  1
\\
& \multicolumn{13}{c}{x_{ij}\in \{0,1\}, \; \forall \, u=(i,j)\in U} &
\end{array}
$$
}
...
\begin{block}{Proposición 8.6.}
	La matriz de incidencia de un digrafo es totalmente unimodular.
\end{block}
	
	\textbf{Probelma relajado:}
	\begin{align*}
		\min\;\;     & \sum_{u\in U} d_u x_{u} \\
		\textrm{s.a.:\;\;} &   {\bf -A x}={\bf b} \\
		&   x_{u} \geq 0, \; u\in U
...
$$
\begin{array}{rrrrrrrrlc}
\text{Max} & -u_s &  & & & &  & +  u_t & & 
\\
\text{\small s.a.} & -u_s & + u_1 & & & & &  & \leq 5 & (s,1)
\\
& -u_s & & + u_2 &  &  & &  & \leq 4 & (s,2)
\\
&  & -u_1  & + u_2 & & & &  & \leq 4 & (1,2)
...
&  &   &  &    & & -u_5 & +u_t  & \leq 2 & (5,t)
\\[2mm]
& \multicolumn{8}{c}{u_i \text{ \small no restringida en signo } i\in\{s,1,2,3,4,5,t\}} 
\end{array}
$$
...
\begin{frame}{Problema del camino m\'inimo desde todos los nodos}
	\begin{itemize}\itemsep 4mm
...
\begin{frame}
	...
	\begin{itemize}\itemsep 3mm
		\item  Inicialmente, $k=0$,  $u_{ij}^{(0)}=d_{ij}$.
		...
		\end{itemize}
		\item Si la longitud mejora al pasar por $k$, se actualiza el camino m\'inimo: {\color{red} ¿cómo?}
	\end{itemize}
	\item Complejidad $O(n^3)$: $n$ iteraciones,  cada iteraci\'on $n^2$ comparaciones.
	\item En cada iteraci\'on se debe ir guardando la informaci\'on del \'arbol de caminos m\'inimos que se va construyendo: {\color{red} ¿cómo?}
	\end{itemize}
...
\begin{frame}
	...
	\frametitle{Algoritmo de Floyd-Warshall}
	 ...
	 \noindent [Paso 0.] Inicializar:
      \begin{itemize}
        \item Matriz de longitudes mínimas: $U^{(0)}=D$, matriz de distancias, $u^{(0)}_{ij}=d_{ij}$.
        \item Matriz de {\color{red} sucesores}: $S^{(0)}$, $s^{(0)}_{ij}=j$ (sucesor de $i$ en el camino $\mu^{(0)}[i,j]$)
       \end{itemize}
        
		 \noindent [Paso 1.] Para $k=1,2,\dots,n$:
		\begin{itemize}\itemsep 3mm
...
\begin{center}  
  ...
  {\color{red} ¿cómo calcular todos los $u_{ij}^{(k)}$ ``a mano''? 
  \\
  ¿cómo reconstruir los caminos mínimos obtenidos?}
